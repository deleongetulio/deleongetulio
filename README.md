### Getulio Cesar De Leon Fernandez

Economist working on national accounts data at the Central Bank of the
Dominican Republic, currently pursuing a Master's in Big Data Analysis and
Visualization (UNIR). I build data pipelines, dashboards, and automation
that turn messy, large-scale data into decisions.

[Website](https://getuliodeleon.com/) - [LinkedIn](https://www.linkedin.com/in/getuliodeleon/)

#### Projects

- **[big-data-msc-portfolio](https://github.com/deleongetulio/big-data-msc-portfolio)** - coursework portfolio: MongoDB, Neo4j, ML, inferential statistics, Spark/HDFS/Kafka.
- **[obsidian-rag-brain](https://github.com/deleongetulio/obsidian-rag-brain)** - local RAG + maintenance scripts for a personal knowledge base.
- **[nominas-sector-publico-rd](https://github.com/deleongetulio/nominas-sector-publico-rd)** - public-sector payroll pipeline for the Dominican Republic (PDF extraction, inflation-adjusted analysis, dashboard).
- **[finanzas-bpd](https://github.com/deleongetulio/finanzas-bpd)** - Gmail-to-Firefly III personal finance automation with LLM categorization.
- **[claude-code-skills](https://github.com/deleongetulio/claude-code-skills)** - 11 Claude Code skills for research, writing, and personal knowledge management.

#### Stack

Python - R - SQL - Power BI - Spark - Streamlit - LaTeX
